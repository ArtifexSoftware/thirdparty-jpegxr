...
typedef struct FILE_OR_MEM
{
  FILE *file;
  unsigned char *start, *pos, *end;
} FILE_OR_MEM;

static size_t mfread(void *ptr, size_t size, size_t nmemb, FILE_OR_MEM *fm)
{
  if (fm->file)
...
static int mfseek(FILE_OR_MEM *fm, int offset, int whence)
{
  if (fm->file)
...
static int read_ifd(jxr_container_t container, FILE_OR_MEM*fm, int image_number, uint32_t*ifd_next);
...
static int _jxr_read_image_container_aux(jxr_container_t container, FILE_OR_MEM*fm)
{
    unsigned char buf[4];
    size_t rc;
