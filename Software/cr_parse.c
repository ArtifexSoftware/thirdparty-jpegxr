...
static int read_ifd_memory(jxr_container_t container, void *data, int size, uint32_t offset, int image_number, uint32_t*ifd_next);
...
int jxr_read_image_container_memory(jxr_container_t container, void *data, int size)
{
    ...
    while (ifd_off != 0) {
        ...
        uint32_t ifd_next;
        if (ifd_off & 0x1) return JXR_EC_IO;
	if (end - start < ifd_off) return (int) -1;
	p = start + ifd_off;
        rc = read_ifd_memory(container, data, size, ifd_off, container->image_count-1, &ifd_next);
        if (rc < 0) return (int) rc;
...
static int read_ifd_memory(jxr_container_t container, void *data, int size, uint32_t offset, int image_number, uint32_t*ifd_next)
{
    ...
    *ifd_next = 0;
    ...
    for (idx = 0 ; idx < entry_count ; idx += 1) {
		...
		if (end - start < ifd_off) return -1;
		p = start + ifd_off;
                cur[idx].value_.p_byte = (uint8_t*)malloc(cur[idx].cnt);

		if (end - p < cur[idx].cnt) return -1;
		memmove(cur[idx].value_.p_byte, p, cur[idx].cnt);
		p += cur[idx].cnt;
#if defined(DETAILED_DBG)
                int bb;
                for (bb = 0 ; bb < cur[idx].cnt ; bb += 1)
